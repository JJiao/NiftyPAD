...
def image_to_parametric(pet_image, dt, model, km_inputs, km_outputs, thr=0.005):
    parametric_images = []
    ...
    for i in range(mask.shape[0]):
        ...
        getattr(tac, 'run_' + model)(**km_inputs)
        pet_image_fit[mask[i][0], mask[i][1], mask[i][2], ] = tac.tacf
        for p in range(len(km_outputs)):
            parametric_images[p][mask[i][0], mask[i][1], mask[i][2], ] = getattr(tac, km_outputs[p].lower())
