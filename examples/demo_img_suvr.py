...
image_suvr = image_to_suvr_with_reference_tac(pet_image=pet_image, dt=dt, reference_regional_tac=ref.tac,
                                              selected_frame_index=frame_index[-2:])
nib.save(nib.Nifti1Image(image_suvr, img.affine), os.path.splitext(img.get_filename())[0] +
             '_suvr' + os.path.splitext(img.get_filename())[1])